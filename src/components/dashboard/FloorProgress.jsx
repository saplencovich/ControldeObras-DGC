...
function BuildingVisual({ floors }) {
  ...
  return (
    <div className="flex flex-col items-center gap-0.5 select-none">
      ...
      <div className="mt-3 flex flex-col gap-1.5 text-[10px] sm:text-xs text-muted-foreground">
        {[
          ...
          <div key={item.label} className="flex items-center gap-1.5">
            <div className={`w-3 h-3 shrink-0 rounded-sm ${item.bg}`} />
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50 border-b-2 border-border hover:bg-muted/50">
                  <TableHead className={cn('text-xs font-semibold text-foreground/90 align-middle', stickyHeadPiso)}>
                    Piso
                  </TableHead>
                  <TableHead className={cn('text-xs font-semibold text-foreground/90 align-middle', stickyHeadInicio)}>
                    F. Inicio
                  </TableHead>
                  <TableHead className="text-xs font-semibold text-foreground/90 text-center">Global</TableHead>
                  {allActivities.map((act) => (
                    <TableHead key={act} className="text-xs font-semibold text-foreground/90 text-center">
                      {act}
                    </TableHead>
                  ))}
                </TableRow>
                ...
                {floorsSorted.map((f) => {
                  ...
                  return (
                    <TableRow key={f.floor} className="group hover:bg-muted/30 border-b">
                      ...
                      <TableCell className={cn('text-xs text-muted-foreground md:group-hover:bg-muted/40', stickyInicio)}>
                        {f.startDate || '—'}
                      </TableCell>
