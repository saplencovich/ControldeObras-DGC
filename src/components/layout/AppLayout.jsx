...
export default function AppLayout() {
  ...
  ...(isAdmin ? [{ path: '/users', label: 'Usuarios', icon: Shield }] : [])];
  ...
  return (
    <div className="min-h-screen bg-background font-inter">
        ...
        <div className="flex items-center justify-between px-4 md:px-6 h-14">
          <div className="flex items-center gap-3">
             ...
             <div className="hidden sm:block">
               <h1 className="text-sm font-semibold leading-tight">Control de Actividades y Avance</h1>
               <p className="text-xs text-primary-foreground/60">Obra Eléctrica</p>
             </div>
           </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10">
              ...
              <span className="text-xs font-medium">{user?.full_name || 'Supervisor'}</span>
            </div>
            <Button
              variant="ghost"
              size="icon"
      ...
      <div className="fixed inset-0 z-40 md:hidden" onClick={() => setMobileOpen(false)}>
          ...
          <nav className="absolute left-0 top-14 w-64 bg-card shadow-xl p-4 space-y-1" onClick={(e) => e.stopPropagation()}>
            {navItems.map((item) => {
            const active = location.pathname === item.path;
            return (
              <Link
